...
class Player {
  ...
  get denomText() {
    if (this.stripes) return 'Bolas con rayas';
    if (this.solids) return 'Bolas solidas';
    return '';
  }

  get nameText() {
    if (this.number === 1) return '<strong>Equipo A</strong>';
    return '<strong>Equipo B</strong>';
  }

  get turnText() {
    let txt = this.number === 1 ? '🙊​ Es tu' : 'Espera!! 👻​ Ya no es tu​';
...
class Canvas {
  ...
  drawSlate() {
    ...
    grad.addColorStop(0, 'rgba(35, 79, 0)');
    grad.addColorStop(0.25, 'rgba(35, 79, 0)');
    grad.addColorStop(1, 'rgba(41, 114, 1 )');
  ...
  drawIndicator({ x, y, cueball, power, maxDistance }) {
    ...
    // max power
    this.context.strokeStyle = 'rgba(93, 93, 93 )';
    this.context.lineWidth = 4;
    this.context.beginPath();
    ...
    // power level
    this.context.strokeStyle = 'rgba(207, 209, 205)';
    this.context.lineWidth = 4;
    this.context.beginPath();
    this.context.moveTo(lineMinX, lineMinY);
  ...
  drawBall(ball) {
    ...
    this.context.shadowColor = 'rgba(0, 0, 0 )';
...
you
  ...
  handleGameOver() {
    ...
    $button.innerHTML = 'Jugar nuevamente';
